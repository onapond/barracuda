...
const storePage = {
  hero: {
    eyebrow: "스토어",
    title: "바라쿠다 제품 프리뷰",
    description: "원두, 굿즈, 라이프스타일 제품을 미리 봅니다.",
  ...
  intro: {
    title: "이 페이지는 프리뷰입니다.",
    description: "구매보다 방향을 먼저 보여주는 스토어 구성입니다.",
  ...
  featuredProduct: {
    ...
    description: "바라쿠다의 톤을 가장 또렷하게 보여주는 중심 블렌드입니다.",
...
export default function StorePage() {
  return (
    <SiteShell>
      ...
      <SectionWrapper><Container className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-start"><div><p className="text-xs uppercase tracking-[0.3em] text-[var(--color-muted)]">스토어 방향</p><h2 className="mt-4 max-w-[12ch] font-display text-[clamp(1.875rem,3vw,2.625rem)] font-medium leading-[1.28] tracking-[-0.02em] [word-break:keep-all]">{storePage.intro.title}</h2><p className="mt-6 max-w-[34ch] text-base leading-[1.75] tracking-[-0.01em] text-[var(--color-muted)] sm:text-lg">{storePage.intro.description}</p></div><div className="rounded-[2rem] border border-[var(--color-line)] bg-[var(--color-surface)] p-6"><span className="rounded-full border border-[var(--color-line)] px-4 py-2 text-xs uppercase tracking-[0.24em] text-[var(--color-muted)]">{storePage.note}</span><div className="mt-6 space-y-4">{storePage.intro.points.map((point) => <div key={point} className="border-t border-[var(--color-line)] pt-4"><p className="text-sm uppercase tracking-[0.22em] text-[var(--color-foreground)]">{point}</p></div>)}</div></div></Container></SectionWrapper>
      <SectionWrapper id="collections" className="bg-[var(--color-surface)]"><Container><div className="flex flex-col gap-4 border-b border-[var(--color-line)] pb-8 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[0.3em] text-[var(--color-muted)]">컬렉션</p><h2 className="mt-4 max-w-[12ch] font-display text-[clamp(1.875rem,3vw,2.625rem)] font-medium leading-[1.28] tracking-[-0.02em] [word-break:keep-all]">카테고리 중심의 제품 구성</h2></div><p className="max-w-[34ch] text-sm leading-[1.75] tracking-[-0.01em] text-[var(--color-muted)] sm:text-right sm:text-base">판매 기능보다 제품 방향을 보여주는 프리뷰에 집중합니다.</p></div><div className="mt-10 grid gap-5 lg:grid-cols-2">{storePage.collections.map((collection, index) => <article key={collection.id} id={collection.id} className="rounded-[2rem] border border-[var(--color-line)] bg-[var(--color-background)] p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.28em] text-[var(--color-muted)]">{collection.eyebrow}</p><h3 className="mt-3 font-display text-3xl leading-tight sm:text-4xl">{collection.title}</h3></div><span className="font-display text-5xl leading-none text-[var(--color-line)]">{String(index + 1).padStart(2, "0")}</span></div><p className="mt-5 max-w-[28ch] text-base leading-[1.75] tracking-[-0.01em] text-[var(--color-muted)]">{collection.description}</p><p className="mt-4 text-sm uppercase tracking-[0.22em] text-[var(--color-foreground)]">{collection.note}</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{collection.items.map((item) => <div key={item} className="rounded-[1.25rem] border border-[var(--color-line)] px-4 py-4 text-sm uppercase tracking-[0.16em]">{item}</div>)}</div></article>)}</div></Container></SectionWrapper>
